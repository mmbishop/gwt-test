...
### Fixed
- When an exception is declared via ```expectingException``` but no exception was thrown during the test, and there is no `then` clause in the test, the test was passing.
...
### Fixed
- Refactored the code so that ```GwtTest``` is no longer a God class.
